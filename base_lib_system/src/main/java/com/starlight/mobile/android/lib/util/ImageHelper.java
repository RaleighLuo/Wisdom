...
public class ImageHelper {
    final int MAX_IMAGE_SIZE=200;//最大图片大小200KB
    ...
    private String compress(Bitmap image,String photoDir,String fileName) {
        String path=null;
        try {
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            image.compress(Bitmap.CompressFormat.JPEG, 100, baos);//质量压缩方法，这里100表示不压缩，把压缩后的数据存放到baos中
     ...
     * @return
     */
    public  String compressImage(String srcPath,String desPhotoDir,String fileName) {
        String path=null;
        try{
            int degree = ViewUtil.getExifOrientation(srcPath);
            BitmapFactory.Options newOpts = new BitmapFactory.Options();
