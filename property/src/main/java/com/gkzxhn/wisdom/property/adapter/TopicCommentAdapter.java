...
public class TopicCommentAdapter extends RecyclerView.Adapter<TopicCommentAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        if(position==0){
            if(mTopicInfor!=null) {
                ImageLoader.getInstance().displayImage(mTopicInfor.getPortrait(), holder.ivHeaderPortrait, Utils.getOptions(R.mipmap.portrait));
            ...
            //下载头像
            ImageLoader.getInstance().displayImage(entity.getPortrait(),holder.ivPortrait,Utils.getOptions(R.mipmap.portrait));
